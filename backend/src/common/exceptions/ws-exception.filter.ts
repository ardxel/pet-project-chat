 ...
 * @publicApi
 */
export class BaseWsExceptionFilter<TError = any> implements WsExceptionFilter<TError> {
  ...
  public handleError<TClient extends { emit: Function }>(client: TClient, exception: TError) {
  ...
  public handleUnknownError<TClient extends { emit: Function }>(exception: TError, client: TClient) {
    const status = 'error';
